package com.netease.is.ac.rp.demo.web;
...
import com.netease.is.ac.rp.sdk.CheckResult;
import com.netease.is.ac.rp.sdk.RegisterProtectionChecker;
...
public class RegisterServlet extends HttpServlet {
    ...
    private void handleCheckSuspect(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            doResponse(response, 401, "注册失败，并增加二次验证（比如：图形验证码、短信验证码等等）");
        } catch (Exception e) {
            // log
            e.printStackTrace();
        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            params.put("nickname", "请替换成用户的昵称");
            params.put("ip", "请替换成用户注册使用的IP");
            */

            // 注册保护检测
            CheckResult result = new RegisterProtectionChecker(SECRET_ID, SECRET_KEY, BUSINESS_ID).check(token, businessParams);
            switch (result) {
                case NORMAL:
